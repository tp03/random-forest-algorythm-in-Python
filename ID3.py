...
class ID3:
    ...
    def __init__(self, input_data, classification_type):
        ...
        if len(input_data) > 0:
            ...
            self.answers, self.attributes = remove_ans(
                self.data_list, attributes_columns=True
            )
            self.most_common_answer = Counter(self.answers).most_common(1)[0][0]
            self.root = self.induce(
                list(range(len(self.attributes))),
                self.attributes,
                self.answers,
                list(range(len(self.answers))),
            )
    ...
    def branchesSetEntropy(self, attribute_values_column, answers):
        ...
        for attr_val in diffAttr:
            sum += (diffAttrAnsSum[attr_val] / len(answers)) * self.entropy(
                diffAttr[attr_val]
            )
        return sum

    def infGain(self, answers, attribute_values_column):
        return self.entropy(answers) - self.branchesSetEntropy(
            attribute_values_column, answers
        )
    ...
    def induce(
        self, attribute_ids, full_attribute_values, full_answers_values, answers_ids
    ):
        attribute_values, answers_values = self.getUsedAttrAndAnswers(
            answers_ids, full_answers_values, full_attribute_values
        )
        ...
        for attr_val, new_answers_ids in new_branches.items():
            child_node = self.induce(
                new_attribute_ids,
                full_attribute_values,
                full_answers_values,
                new_answers_ids,
            )
    ...
    def randomize(self, infGains):
        # dodanie losowego ograniczania zbioru argumentów przy tworzeniu gałęzi (element RandomForest)
    ...
    def roulette(self, gains):
        ...
        for gain in gains.values():
            if gain > 0 and self.classify_method == "entropy":
                # prawdopodobieństwo wyboru testu jako stosunek jego przyrostu informacji do sumy przyrostów
                probabilities.append(gain / sum_of_gains)
            elif gain > 0 and self.classify_method == "gini":
                # jako że przy wsp. Ginniego docelowo wybieramy najmniejszą wartość funcji celu,
                # zaimplementowano prawdopodobieństwo jako odwrotność stosunku funkcji celu do sumy funkcji celu
                probabilities.append(sum_of_gains / gain)
            else:
                probabilities.append(0.0)
        # normalizacja prawdopodobieństw (kluczowe przy metodzie Ginni)
        probabilities = [p / sum(probabilities) for p in probabilities]
        return np.random.choice(list(gains.keys()), 1, False, p=probabilities)[0]
